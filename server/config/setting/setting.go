...
func (s *Setting) InitSetting() {
	...
	yamlPath := filepath.Join(homeDir, "yixiyilu", "config.yaml")
...
func (b *BlogVariable) InitBlogVars(siteSetting map[string]string, socialInfo map[string]string) {
	...
	if staticRoot, ok := siteSetting["static_root"]; ok {
		...
		if Config.Server.Mode == gin.DebugMode {
			b.StaticRootPath = b.ContextPath + "/resources"
		} else {
			b.StaticRootPath = "https://jsd.cdn.zzko.cn/gh/yixiyilu/resources@master"
		}
	}
	...
	if siteDesc, ok := siteSetting["site_desc"]; ok {
		...
		b.SiteDesc = "叹驹中隙，石中火，梦中影；对一张琴，一壶酒，一溪云 | 一溪一炉 | www.yixiyilu.com"
	}
	...
	if siteLogo, ok := siteSetting["site_logo"]; ok {
		...
		b.SiteLogo = "https://s2.loli.net/2024/06/27/VplXYOcQv5T6r9k.png"
	}
